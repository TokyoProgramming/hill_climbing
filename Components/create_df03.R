create_df03 <- function(A, D=100, kurikaeshi= 20, show=4,set_bic=1000, whiteList=0, blackList=0) {
  
  col_num <- ncol(A)
  row_num <- col_num
  
  res.BIC_array <- rep(0, kurikaeshi)
  res.AIC_array <- rep(0, kurikaeshi)
  
  res.path_array <- array(0, dim=c(row_num,col_num,kurikaeshi))
  
  
  # Read b_hill_climbing03s
  source('b_hill_climbing_03s.r')
  
  df <- data.frame(id = as.integer(), AIC = numeric(), BIC =numeric(), dup=as.integer())
  k <- 1

  repeat{
    hill_climbing <- b_hill_climbing(A,D,set_bic, whiteList, blackList,silent=T)
    
    res.BIC <- hill_climbing$res$BIC
    res.AIC <- hill_climbing$res$AIC
    res.path <- hill_climbing$Path_hc
    
    res.path_array[,,k] <- res.path
    res.BIC_array[k] <- res.BIC
    res.AIC_array[k] <- res.AIC
    
    # Read dag_check02
    source('dag_check02.r')
    if(dag_check(res.path) == 1){
      # If res.path == DAG
      
      large_BIC <- df$BIC[show]
      
      if(is.na(large_BIC) == TRUE){
        # Data frame's row number is smaller than show Number 

        if((res.BIC %in% df$BIC) == FALSE){    
          # there is No-identical BIC Number in the data frame
          
          # Add the data to the df
          last_row <- nrow(df) + 1 
          df[last_row, ] = c(k, res.AIC, res.BIC, 0)

          # sort the df  
          df1 <- df[order(df$BIC),]
          df <- df1
          
          # if nrow(df) is larger than show delete the last row
          row_Num <- nrow(df)
          if(show < row_Num){
            last_row <- nrow(df)
            df <- df[c(-last_row),]
          }

        }

        
      }else{
        # compare large_BIC and res.BIC
        if(res.BIC < large_BIC){
          # res.BIC < large_BIC
          # identical BIC is in the data frame or not 
          
          if((res.BIC %in% df$BIC) == TRUE){
            # there is the identical BIC Number in the data frame
            
            # 
            # 1. check : if there are multiple identical BIC numbers
            # 
            df_BIC_v <- as.double(df$BIC)
            df_id    <- which(res.BIC == df_BIC_v)
            df_id_len <- length(df_id)
            
            ide <- 1
            j <- 1 
            for (x in 1:df_id_len){
              id_location <- df_id[x]
              
              if(identical(res.path,res.path_array[,,id_location]) == TRUE){
                
                # If path_matrices are  identical
                
                
                # Get row number of id_location in the data frame
                df.id_num <- which(grepl(id_location, df$id))
                # add dup (duplicate Number)
                new_dup <- as.double(df$dup[df.id_num]) + 1
                # Replace the dup element 
                dup_col <- ncol(df)
                df[df.id_num, dup_col] = new_dup
                
              }else{
                # if all path matrices are not identical
                
                if(j == df_id_len){
                  
                  last_row <- nrow(df) + 1 
                  df[last_row, ] = c(k, res.AIC, res.BIC, 0)
                  
                  
                  # sort the df  
                  df1 <- df[order(df$BIC),]
                  df <- df1
                  
                  # if nrow(df) is larger than show delete the last row
                  row_Num <- nrow(df)
                  if(show < row_Num){
                    last_row <- nrow(df)
                    df <- df[c(-last_row),]
                  }
                  
                }
                
              }
              
              j <- j + 1
              
            }
            
          }else{
            # there is No-identical BIC Number in the data frame
            
            # Add the data to the df
            last_row <- nrow(df) + 1 
            df[last_row, ] = c(k, res.AIC, res.BIC, 0)
            
            # sort the df  
            df1 <- df[order(df$BIC),]
            df <- df1
            
            # if nrow(df) is larger than show delete the last row
            row_Num <- nrow(df)
            if(show < row_Num){
              last_row <- nrow(df)
              df <- df[c(-last_row),]
            }
            
          }
          
          
          
        } 
        


      }
      


    }

    if(k == kurikaeshi){
      break
    } 
    
    k <- k + 1
    

  }
  
  result_df <- head(df,show)
  return(list(k=k,result_df=result_df, large_BIC=large_BIC))
  
}




















