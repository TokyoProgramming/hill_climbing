b_hill_climbing_02<-function(A,D=100, set_bic = 1000, whiteList=0, blackList=0){
  ...
  agfi <- AGFI(saiteki_hc$BVar,cor(A),n,N)
  gfi <- GFI(saiteki_hc$BVar,cor(A))
  cat("AGFI	=",agfi,"	GFI	=",gfi,"\n")
  cat("--------------------------------------------\n")
  print(saiteki_hc)
  invisible(c(saiteki_hc,list(ChiSq=ki_hc,DF=N,PV=pv,AIC=aic,BIC=bic,AGFI=agfi,GFI=gfi)))
