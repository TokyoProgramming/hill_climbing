create_df04 <- function(A, D=100, kurikaeshi= 300, show=15,set_bic=1000, whiteList=0, blackList=0){
      ...
      if((res.BIC %in% as.double(df$BIC)) == TRUE){
        ...
        for (x in 1:df_id_len){
          ...
          if(identical(res.path,res.path_array[,,id_location]) == TRUE){
            ...
            df.id_num <- which(id_location == df$id)
            ...
            if(x == df_id_len && j == 1){
              ...
              if(nrow(df) < show){
                add_row <- nrow(df) + 1
                df[add_row,] = c(k, res.AIC, res.BIC, 1)
